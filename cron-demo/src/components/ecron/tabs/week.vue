  ...
  <div class="config-list">
    <a-radio-group v-model="type">
      ...
      <div class="item">
        ...
        从<a-select v-model="valueRange.start"  class="w80" :disabled="type!=TYPE_RANGE || disableChoice">
          <a-select-option v-for="(v, k) of WEEK_MAP" :value="v" :key="`week-pre-Lf13-${v}`">{{ k }}</a-select-option>
        </a-select>
        至<a-select v-model="valueRange.end"  class="w80" :disabled="type!=TYPE_RANGE || disableChoice">
          <a-select-option v-for="(v, k) of WEEK_MAP" :value="v" :key="`week-next-1fas-${v}`">{{ k }}</a-select-option>
        </a-select>
      </div>
      <div class="item">
        <a-radio value="TYPE_LOOP" class="choice" :disabled="disableChoice">循环</a-radio>
        从<a-select v-model="valueLoop.start"  class="w80" :disabled="type!=TYPE_LOOP || disableChoice">
          <a-select-option v-for="(v, k) of WEEK_MAP" :value="v" :key="`week-pre-Lf13-${v}`">{{ k }}</a-select-option>
        </a-select>开始，间隔
      ...
      <div class="item">
        ...
        <div class="list">
          <a-checkbox-group v-model="valueList">
            <a-checkbox class="list-check-item" v-for="(v, k) of WEEK_MAP"
                        :value="v" :key="`key-01jfs-${v}`" :disabled="type!=TYPE_SPECIFY || disableChoice">
              <span>{{k}}</span>
            </a-checkbox>
          </a-checkbox-group>
        </div>
